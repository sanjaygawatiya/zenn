...
import { CameraIR, CameraProgramItem } from '../ir/camera.js';
...
export interface TimelineToCameraInput {
  readonly storyboard: StoryboardIR;
  readonly layout: LayoutIR;
  readonly timeline: TimelineIR;
}

export class TimelineToCameraCompiler implements CompilerPass<TimelineToCameraInput, CameraIR> {
  readonly id = 'CP-003';
  readonly version = '1.0';

  compile(
    input: Readonly<TimelineToCameraInput>,
    context: CompilerContext
  ): Result<Readonly<CameraIR>, CompilerError[]> {
    ...
    const initialItem: CameraProgramItem = {
      programId: 'CAM-0001',
      startMs: 0,
    ...
    if (shouldAddMove) {
      ...
      const moveItem: CameraProgramItem = {
        programId: 'CAM-0002',
        startMs: 1000,
